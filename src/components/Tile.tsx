import { TileType } from '../types'

type Props = {
  tile: TileType
  back: string
  onClick: () => void
}

export default function Tile({ tile, back, onClick }: Props) {
  return (
    <button
      onClick={onClick}
      disabled={tile.flipped || tile.matched}
      className='perspective-1000 preserve-3d relative aspect-square w-full transform overflow-hidden rounded-lg border p-1 transition-transform focus:outline-none disabled:opacity-60'
      aria-pressed={tile.flipped || tile.matched}>
      <div
        className={`backface-hidden absolute bottom-0 left-0 right-0 top-0 transition-transform duration-300 ${
          tile.flipped || tile.matched ? 'rotate-y-0' : 'rotate-y-180'
        }`}>
        <img src={tile.image} alt='front' className='h-full w-full object-cover' />
      </div>
      <div
        className={`backface-hidden absolute bottom-0 left-0 right-0 top-0 transition-transform duration-300 ${
          tile.flipped || tile.matched ? 'rotate-y-180' : 'rotate-y-0'
        }`}>
        <img src={back} alt='back' className='h-full w-full object-cover' />
      </div>
    </button>
  )
}
